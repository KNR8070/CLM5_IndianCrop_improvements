  ...
  subroutine CropPhenology(num_pcropp, filter_pcropp                     , &
    ...
    associate(                                                                   & 
      ...
      do fp = 1, num_pcropp
         ...
         if ( jday == jdayyrstart(h) .and. mcsec == 0 )then  ! 1
         ...
         if ( (.not. croplive(p)) .and. (.not. cropplant(p)) ) then      ! 1
            ...
            if (ivt(p) == nwwheat .or. ivt(p) == nirrig_wwheat) then  ! 2      
               ...
               if (a5tmin(p)             /= spval                  .and. &
                  ...
                  if (use_c13) then    ! 4
                     if ( cnveg_carbonstate_inst%totvegc_patch(p) .gt. 0._r8) then  ! 5
                  ...
                  ! crops were harvested for that year
                    ...
                    gdd020(p)  /= spval                   .and. &
                    gdd020(p)  >= gddmin(ivt(p))) then   !3 Cont.
            ...
            else ! not winter cereal... slevis: added distinction between NH and SH        ! 2 Cont.
               ...
               if (t10(p) /= spval.and. a10tmin(p) /= spval   .and. &
                  ...
                  if (ivt(p) == ntmp_soybean .or. ivt(p) == nirrig_tmp_soybean .or. &
                       ivt(p) == ntrp_soybean .or. ivt(p) == nirrig_trp_soybean) then  ! 4
                  ...
                  if (ivt(p) == ntmp_corn .or. ivt(p) == nirrig_tmp_corn .or. &
                      ...
                      ivt(p) == nsugarcane .or. ivt(p) == nirrig_sugarcane) then! 4
                  ...
                  if (ivt(p) == nswheat .or. ivt(p) == nirrig_swheat .or. &
                      ...
                      ivt(p) == nrice   .or. ivt(p) == nirrig_rice) then ! 4
                  ...
                  ! pools here.  use totvegc_patch as the closest analogue if nonzero, and use initial value otherwise
                  if (use_c13) then    ! 5
                     if ( cnveg_carbonstate_inst%totvegc_patch(p) .gt. 0._r8) then ! 6
                        c13_cnveg_carbonstate_inst%leafc_xfer_patch(p) = leafc_xfer(p) * &
                             c13_cnveg_carbonstate_inst%totvegc_patch(p) / cnveg_carbonstate_inst%totvegc_patch(p)
                     else
                        c13_cnveg_carbonstate_inst%leafc_xfer_patch(p) = leafc_xfer(p) * c13ratio
                  ...
                  if (ivt(p) == ntmp_soybean .or. ivt(p) == nirrig_tmp_soybean .or. &
                      ivt(p) == ntrp_soybean .or. ivt(p) == nirrig_trp_soybean) then ! 5
                     gddmaturity(p) = min(gdd1020(p), hybgdd(ivt(p)))
                  end if   ! 5
                  if (ivt(p) == ntmp_corn .or. ivt(p) == nirrig_tmp_corn .or. &
                      ivt(p) == ntrp_corn .or. ivt(p) == nirrig_trp_corn .or. &
                      ivt(p) == nsugarcane .or. ivt(p) == nirrig_sugarcane)then ! 5
                     gddmaturity(p) = max(950._r8, min(gdd820(p)*0.85_r8, hybgdd(ivt(p))))
                  end if  ! 5
                  if (ivt(p) == nswheat .or. ivt(p) == nirrig_swheat .or. &
                      ivt(p) == ncotton .or. ivt(p) == nirrig_cotton .or. &
                      ivt(p) == nrice   .or. ivt(p) == nirrig_rice) then ! 5
                     gddmaturity(p) = min(gdd020(p), hybgdd(ivt(p)))
                  end if  ! 5
            ...
            end if ! crop patch distinction     ! 3
            ...
            if (ivt(p) == ntmp_corn .or. ivt(p) == nirrig_tmp_corn .or. &
                ivt(p) == ntrp_corn .or. ivt(p) == nirrig_trp_corn .or. &
                ivt(p) == nsugarcane .or. ivt(p) == nirrig_sugarcane) then   ! 3
         ...
         end if ! crop not live nor planted                       ! 2
         ...
         if (croplive(p)) then                              ! 2
            ...
            if (t_ref2m_min(p) < 1.e30_r8 .and. vf(p) /= 1._r8 .and. &
               (ivt(p) == nwwheat .or. ivt(p) == nirrig_wwheat)) then  ! 3
            ...
            else if (hui(p) >= gddmaturity(p) .or. idpp >= mxmat(ivt(p))) then ! 2
               if (harvdate(p) >= NOT_Harvested) harvdate(p) = jday      !Harvest Block
               croplive(p) = .false.     ! no re-entry in greater if-block
               cphase(p) = 4._r8
               if (tlai(p) > 0._r8) then ! plant had emerged before harvest  ! 3  
                  offset_flag(p) = 1._r8
                  offset_counter(p) = dt
               else                      ! plant never emerged from the ground  ! 3 Cont.
            ...
            else if (hui(p) >= huigrain(p)) then         ! 2 Cont..
               cphase(p) = 3._r8
               bglfr(p) = 1._r8/(leaf_long(ivt(p))*dayspyr*secspday)
            end if                                       ! 2
 !!!!!!!!!!!!!!!!!!!!!! KNR_20220914 !!!!!!!!!!!!!!!!!!!!!!!!!!
! If crop is growing in 2 calender years, reset cropplant to sow again in the
! second calender year
            if (cphase(p) == 4._r8) then ! Enter the loop if crop phase is harvested
               if (idop(p) > jday) then  ! Enters if crop is planted in the last calender year 
                  cropplant(p) = .false.    ! Sets crop to not planted in this calender year
                  idop(p) = NOT_PLANTED
               else
                  cropplant(p) = .true.     
               end if
